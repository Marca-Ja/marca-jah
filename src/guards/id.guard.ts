  ...
  export class IdGuard implements CanActivate {
    ...
    async canActivate(context: ExecutionContext) {
      ...
      try {
        ...
        if (!userId) {
            throw new UnauthorizedException('User identifier parameter not found in the request.');
        }
        
        if (data.sub !== userId) {
          throw new UnauthorizedException('Access denied: User can only access their own profile.');
        }
